...
def get_remote_database_url():
    config_path = os.path.join(settings.BASE_DIR, 'db_config.json')
    try:
        if os.path.exists(config_path):
            with open(config_path, 'r') as f:
                config = json.load(f)
                return config.get("REMOTE_DATABASE_URL")
        else:
            print("⚠️ db_config.json not found.")
            return None
    except Exception as e:
        print(f"❌ Failed to fetch 'REMOTE_DATABASE_URL': {e}")
        return None


def configure_online_database():
    url = get_remote_database_url()
    if not url:
...
def apply_deletions():
    Deleted_Record = apps.get_model("sync", "Deleted_Record")
    print("\n🗑️ Applying Deletions...")

    for source, target in [('default', 'online'), ('online', 'default')]:
        print(f"\n🔁 Deletion Pass: {source} ➝ {target}")
        # Deleted_Record.objects.using(source).all().delete()
        records = Deleted_Record.objects.using(source).filter(deleted=0)
        for record in records:
            try:
                model = apps.get_model(record.app_name, record.model_name)
                obj = model.objects.using(target).filter(id=record.model_id).first()
                if obj:
                    obj.delete()
                    print(f"✅ Deleted {record.model_name} ({record.model_id}) in {target}")
                else:
                    print(f"⚠️ {record.model_name} ({record.model_id}) not found in {target}")
            except Exception as e:
                print(f"❌ Error deleting {record.model_name} ({record.model_id}) in {target}: {e}")

        # Optional: mark deletions as applied
        records.update(deleted=1)
...
def sync_model(model_class):
    ...
    try:
        offline_latest_sync_offline = model_class.objects.using('default')\
            .exclude(sync_offline__isnull=True).order_by('-sync_offline')\
            .values_list('sync_offline', flat=True).first() or 0

        online_latest_sync_offline = model_class.objects.using('online')\
            .exclude(sync_offline__isnull=True).order_by('-sync_offline')\
            .values_list('sync_offline', flat=True).first() or 0

        offline_latest_sync_online = model_class.objects.using('default')\
            .exclude(sync_online__isnull=True).order_by('-sync_online')\
            .values_list('sync_online', flat=True).first() or 0

        online_latest_sync_online = model_class.objects.using('online')\
            .exclude(sync_online__isnull=True).order_by('-sync_online')\
            .values_list('sync_online', flat=True).first() or 0
    ...
    try:
        offline_ids_to_upload = model_class.objects.using('default')\
            ...
            .values_list(pk_field, flat=True).order_by('sync_offline')
    except Exception as e:
        print(f"❌ Upload filter failed: {e}")
        return

    if offline_ids_to_upload:
        print(f"⬆️ Uploading {len(offline_ids_to_upload)} record(s) where offline.sync_offline > online.sync_offline...")
        for obj_id in offline_ids_to_upload:
            try:
                local_obj = model_class.objects.using('default').get(**{pk_field: obj_id})
                kwargs = build_create_kwargs(local_obj)
                online_exists = model_class.objects.using('online').filter(**{pk_field: obj_id}).exists()
                if not online_exists:
                    model_class.objects.using('online').create(**kwargs)
                    print(f"⬆️ Created: {obj_id}")
                else:
                    model_class.objects.using('online').filter(**{pk_field: obj_id}).update(**kwargs)
                    print(f"⬆️ Updated: {obj_id}")
            except ObjectDoesNotExist as e:
                print(f"❌ FK missing during upload [{model_path} ID={obj_id}]: {e}")
                raise  # Stop syncing this model
            except Exception as e:
        ...
        print("✅ No offline records to upload.")

    try:
        online_ids_to_download = model_class.objects.using('online')\
            .filter(sync_online__gt=offline_latest_sync_online)\
            .values_list(pk_field, flat=True).order_by('sync_online')
    except Exception as e:
        print(f"❌ Download filter failed: {e}")
        return

    if online_ids_to_download:
        print(f"⬇️ Downloading {len(online_ids_to_download)} record(s) where online.sync_online > offline.sync_online...")
        for obj_id in online_ids_to_download:
            try:
                online_obj = model_class.objects.using('online').get(**{pk_field: obj_id})
                kwargs = build_create_kwargs(online_obj)
                offline_exists = model_class.objects.using('default').filter(**{pk_field: obj_id}).exists()
                if not offline_exists:
                    model_class.objects.using('default').create(**kwargs)
                    print(f"⬇️ Created: {obj_id}")
                else:
                    model_class.objects.using('default').filter(**{pk_field: obj_id}).update(**kwargs)
                    print(f"⬇️ Updated: {obj_id}")
            except ObjectDoesNotExist as e:
                print(f"❌ FK missing during download [{model_path} ID={obj_id}]: {e}")
                raise  # Stop syncing this model
            except Exception as e:
        ...
        print("✅ No online records to download.")
...
def main():
    ...
    # Step 1: Apply Deletions Both Ways
    apply_deletions()
