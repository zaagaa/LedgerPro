...
class PrintHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            printer._raw(b'\x1B\x70\x00\x19\xFA')  # Open drawer
            printer._raw(b'\n')  # Trigger flush
...
def is_agent_running(port=9999):
    """Check whether another agent is already running."""
...
if __name__ == "__main__":
    ...
    if is_agent_running(PORT):
        print("LedgerPro Print Agent is already running.")
        sys.exit(0)

    run(port=PORT)

# pyinstaller agent.spec #OLD METHOD
