...
:: 2) Check standard folder if not found
...
IF NOT DEFINED PSQL_EXE (
    ...
    IF EXIST "%PG_INSTALLER%" (
        ...
        curl -L -o "%PG_INSTALLER%" "%PG_URL%"
    )

    echo [INFO] Installing PostgreSQL...
    start /wait "" "%PG_INSTALLER%" --mode unattended --unattendedmodeui none --superpassword postgres --servicename postgresql-x64-%PG_VERSION% --serverport 5432
...
IF NOT DEFINED PSQL_EXE (
    ...
    echo [INFO] PostgreSQL detected: %PSQL_EXE%
)
