...
class Pos_Supplier_payment(View):
    ...
    def get(self, request):
        ...
        payment_list=Payment.objects.filter(company_id=request.COOKIES.get('company_id'), user_id=request.user.id, transaction_date=timezone.localtime().date())

        total_amount=payment_list.aggregate(total_amount=Sum('amount', default=0))

        total_amount=total_amount.get('total_amount')

        form_start_date = timezone.localtime().strftime('%d/%m/%Y')
        form_end_date = timezone.localtime().strftime('%d/%m/%Y')
    ...
    def post(self, request):
        ...
        if 'amount' in request.POST:
            Payment(company_id=request.COOKIES.get('company_id'), user_id=request.user.id,
                    ...
                    transaction_date=timezone.localtime(), description='CASH PAID BY CASH COUNTER',
                    paid_by='CASH',
...
class Pos_Bundle(View):
    def get(self,request):
        ...
        bundle_list = Bundle.objects.filter(company_id=request.COOKIES.get('company_id'), user_id=request.user.id,
                                            entry_date__date=timezone.localtime().date())
...
def pos_statement_update(request):
    ...
    print(timezone.localtime().strftime('%d/%m/%Y'))
    ...
    if Cash_Counter.objects.filter(company_id=request.COOKIES.get('company_id'), user_id=request.user.id, entry_date=entry_date).exists():
        try:
            obj = Cash_Counter.objects.get(company_id=request.COOKIES.get('company_id'), user_id=request.user.id, entry_date=entry_date)
            obj.stand_by = stand_by
            obj.cash_taken = cash_taken
            obj.save()
        except Cash_Counter.DoesNotExist:
            pass
...
def pos_bill_list(request):
    ...
    form_invoice_date=timezone.localtime().strftime('%d/%m/%Y')
...
def pos_expenses(request):
    ...
    if Cash_Counter.objects.filter(company_id=request.COOKIES.get('company_id'), user_id=request.user.id,
                                   entry_date__date=timezone.localtime().date(), finish=1).exists():
        finish=1
    if 'amount' in request.POST:
       if finish==0:
    ...
    if 'start_date' in request.POST and 'end_date' in request.POST:
        ...
        # Get expenses safely
        expenses_list = Expenses.objects.filter(
            entry_date__date=timezone.localtime(),
            company_id=request.COOKIES.get('company_id'),
            user_id=request.user.id
        )
...
def pos_statement(request):
    ...
    form_statement_date=timezone.localtime().strftime('%d/%m/%Y')
    statement_date=timezone.localtime().date()
    ...
    try:
        ...
        for data in cancel_bill:
            total_cancel_amount += Invoice.objects.filter(company_id=request.COOKIES.get('company_id'),
                                                          ...
                                                          invoice_date__date=statement_date).latest("created_at").total_amount
    except:
        cancel_bill=None
    ...
    bundle_charge=0
    try:
        bundle_list=Bundle.objects.filter(company_id=request.COOKIES.get('company_id'), user_id=request.user.id, entry_date__date=timezone.localtime().date())
    ...
    if Cash_Counter.objects.filter(company_id=request.COOKIES.get('company_id'), user_id=request.user.id, entry_date=statement_date).exists():
        cash_counter=Cash_Counter.objects.filter(company_id=request.COOKIES.get('company_id'), user_id=request.user.id,
                                    entry_date=statement_date).latest("created_at")
    ...
    if cash_counter_queryset.exists():
        cash_counter_stand_byx = cash_counter_queryset.latest("created_at")  # Get the latest entry
...
def pos_bill_copy(request):
    ...
    try:
        invoice = Invoice.objects.filter(company_id=request.COOKIES.get('company_id'), user_id=request.user.id).latest("created_at")
        context = {
...
def pos_print_bill(request):
    ...
    if request.method == "POST":
        ...
        if Invoice.objects.filter(user_id=request.user.id, company_id=request.COOKIES.get('company_id'), invoice_date__date=invoice_date, invoice_number=request.POST['bill_number']).exists():
            invoice_id=Invoice.objects.filter(company_id=request.COOKIES.get('company_id'), invoice_date__date=invoice_date, invoice_number=request.POST['bill_number']).latest("created_at").id
            ...
            invoice_id = Invoice.objects.filter(company_id=request.COOKIES.get('company_id'),
                                                invoice_number=request.POST['bill_number']).latest("created_at").id
            context = print_invoice_context(invoice_id)
        else:
            context = {}


    if timezone.localtime().date()==invoice_date:
        context["finish"]=0
    else:
        context["finish"] = 1
    if Cash_Counter.objects.filter(company_id=request.COOKIES.get('company_id'), user_id=request.user.id,
...
def pos_cancel_bill(request):
    ...
    if request.method == "POST":
        ...
        try:
            obj = Invoice.objects.get(id=cancel_invoice_id)
            obj.cancel_no = cancel_no
            obj.save()
        ...
        for data in sale_data:
            ...
            data.save()
            try:
                obj = Sale.objects.get(id=data.id)
                obj.invoice_id = cancel_invoice_id
                obj.save()
...
def new_invoice_number(request):
    ...
    if timezone.localtime() < datetime(timezone.localtime().year, 4, 1):
        account_year = datetime(timezone.localtime().year - 1, 4, 1)
    else:
        account_year = datetime(timezone.localtime().year, 4, 1)

    if Invoice.objects.filter(company_id=request.COOKIES.get('company_id'), invoice_date__gte=account_year,
                              invoice_date__lte=timezone.localtime()).exists():
        invoice_number = Invoice.objects.filter(company_id=request.COOKIES.get('company_id'),
                                                invoice_date__gte=account_year,
                                                invoice_date__lte=timezone.localtime()).latest(
...
def add_new_invoice(request):
    ...
    if request.method=="POST":
        ...
        invoice=Invoice.objects.filter(company_id=request.COOKIES.get('company_id')).latest("created_at")
...
def barcode_query(request):
    ...
    if request.method == 'GET':
        ...
        if context is None and len(str(query)) > 2:
            try:
                stock = Stock.objects.filter(pre_barcode__iexact=query).latest("created_at")
