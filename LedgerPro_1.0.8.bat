...
echo.
set /p CHOICE=Select installation drive (Number or Drive Letter): 
...
set "INSTALL_PATH=%DRIVE%:\LedgerPro"
...
if exist "%INSTALL_PATH%" (
    ...
    echo.
    echo LedgerPro already exists.
    echo.

    choice /C YN /M "Remove existing installation"

    :: N = Keep existing installation and skip clone/download
    if errorlevel 2 (
        echo.
        echo Using existing LedgerPro installation...
        goto CREATE_CONFIG
    )

    :: Y = Remove and reinstall
    echo.
    ...
    if exist "%INSTALL_PATH%" (
        echo.
        echo Unable to remove existing folder.
        pause
        exit /b 1
    )
...
IF NOT EXIST %PROJECT_FOLDER% (
    ...
    %GIT_CMD% clone -b %BRANCH_NAME% %REPO_URL% "%INSTALL_PATH%\%PROJECT_FOLDER%"
    IF ERRORLEVEL 1 (
...
IF EXIST "%INSTALL_PATH%\%PROJECT_FOLDER%" (
    ...
    (
        ...
        echo }
    ) > "%INSTALL_PATH%\%PROJECT_FOLDER%\db_config.json"
...
set "VBS_FILE=%INSTALL_PATH%\%PROJECT_FOLDER%\run_startup.vbs"
set "SHORTCUT=%STARTUP_FOLDER%\LedgerPro Startup.lnk"
...
$Shortcut.WorkingDirectory = '%INSTALL_PATH%\%PROJECT_FOLDER%'; ^
$Shortcut.IconLocation = '%SystemRoot%\System32\wscript.exe,0'; ^
$Shortcut.Save()"
...
cd /d "%INSTALL_PATH%\%PROJECT_FOLDER%"
...
:: Start LedgerPro in background
start "" "run_startup.vbs"
...
IF NOT DEFINED APP_FOUND (
    IF EXIST "%LOCALAPPDATA%\Programs\Ledger-Pro\%APP_EXE%" (
        SET "APP_FOUND=%LOCALAPPDATA%\Programs\Ledger-Pro\%APP_EXE%"
    )
)
...
IF NOT DEFINED APP_FOUND (
    ...
    IF EXIST "%LOCALAPPDATA%\Programs\Ledger-Pro\agent.exe" (
        echo [INFO] Removing old installation...

        taskkill /F /IM agent.exe >nul 2>&1
        timeout /t 2 >nul

        rmdir /S /Q "%LOCALAPPDATA%\Programs\Ledger-Pro"
    )

    :: Remove old installer
    IF EXIST "%APP_INSTALLER%" del /f /q "%APP_INSTALLER%" >nul 2>&1

    echo.
    echo [INFO] Downloading LedgerPro Desktop...

    curl.exe -L --fail -o "%APP_INSTALLER%" "%APP_URL%"

    IF ERRORLEVEL 1 (
        echo.
        echo [ERROR] Failed to download LedgerPro installer.
        pause
        exit /b 1
    )

    IF NOT EXIST "%APP_INSTALLER%" (
        echo.
        echo [ERROR] Installer was not downloaded.
        pause
        exit /b 1
    )
...
IF ERRORLEVEL 1 (
    ...
    echo.
    echo [INFO] LedgerPro Desktop is already running.
    exit /b 1
)
...
IF DEFINED APP_FOUND (
    ...
    echo.
    echo [INFO] LedgerPro Desktop found:
    echo %APP_FOUND%
    ...
    echo ERROR
    echo ==========================================
    echo Ledger-Pro.exe could not be found.
    echo Please install LedgerPro manually.
    pause
    exit /b 1
