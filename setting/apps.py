...
class SettingConfig(AppConfig):
    ...
    def ready(self):
        ...
        try:
            ...
            if 'pos_print_template' in connection.introspection.table_names():
                Print_Template = apps.get_model('pos', 'Print_Template')
                ...
                html += `<tr><td colspan="5">Customer Name: `+ (response.customer_name) + `</td></tr>`;
                html += `<tr><td colspan="5">Mobile: `+ (response.customer_mobile) + `</td></tr>`;
                }
                
                html += `<tr><td colspan="5" style="white-space: nowrap;">{{pos_footer|safe}}</td></tr>
                <tr><td colspan="5"><center><span style="font-weight:bold;font-size:24px;">`+ response.qrcode_data + `</span><br>` + qr_image + `</center></td></tr>`;
                
                html +=`</tbody></table>`;"""

                template = Print_Template.objects.filter(pk=DEFAULT_UUID).first()
                ...
                post_delete.connect(
                    recreate_template,
                    sender=Print_Template,
                    dispatch_uid='default_template_recreate'
                )
