...
class ElectronOnlyMiddleware:
    ...
    def __call__(self, request):
        # Only apply restriction if running in offline mode
        if getattr(settings, 'INSTANCE_TYPE', '').lower() == 'offline':
            user = request.user
            if user.is_authenticated and not user.is_superuser:
                if request.headers.get('X-Electron-App') != 'yes':
                    logout_url = '/logout/'  # Adjust if your logout URL is different
                    message = format_html(
